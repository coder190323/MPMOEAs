function varargout = DMOPs2_OS(Operation,Global,input)
% <problem> <DMOPs>
% Dynamic Multiobjective Optimization Problem 2
...
persistent PF;
    ...
    switch Operation
        case 'init'
            ...
            %判断使用的函数strcmp(func2str(Global.algorithm),'NSGAII');
            [PF] = Global.ParameterFile('PF',ones(Global.D,Global.M)*inf);
        ...
        case 'value'
            ...
            for p = 1:N
                ...
                input(p,:) = x;
              	%计算适应度函数
                PopObj(p,1) = gnum(a);              %供油罐个数
                PopObj(p,2) = gchange(a);        %蒸馏塔的供油罐切换次数
                PopObj(p,3) = gdmix(a, c1);      %管道混合成本
                PopObj(p,4) = gdimix(a, c2);     %罐底混合成本
            end
            
            PopCon = [];
            
            varargout = {input,PopObj,PopCon};  %input：决策变量；PopObj：目标函数值；PopCon：约束函数值
        case 'PF'
            RefPoint  = PF;
            varargout = {RefPoint};
    end
...
%% 计算解的适应度
function [ eff,scheduleplan,x ] = fat( pop )
%变量取值范围[0,1]
%决策变量维数w
%目标函数个数M=4
